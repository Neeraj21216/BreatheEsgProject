...
def _build_normalized(row, raw, tenant):
    ...
    raw_qty = (
        row.get("quantity") or row.get("kwh") or
        row.get("consumption") or row.get("usage") or
        row.get("units consumed") or ""
    )
    try:
        qty = Decimal(str(raw_qty).replace(",", "").strip())
    except InvalidOperation:
        errors.append({"field": "quantity", "raw_value": raw_qty, "error": "Not a number"})
